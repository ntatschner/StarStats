...
/** 30-day heatmap. Bins each TimelineBucket into one of 5 levels
 * (0 = empty, 1-4 by quartile-ish thresholds against the bucket max),
 * then lays them out on the calendar grid: rows = day-of-week (Sun
 * → Sat), columns = ISO weeks. The leading column may be partially
 * empty if the window doesn't start on a Sunday — those leading
 * cells render as level 0. */
function DayHeatmap({ timeline }: { timeline: TimelineResponse }) {
